...
def debug_create_objects(object_list):
    ...
    x = 25
    for i in range(0,7):
        block = SingleHit(object_list, SCREEN_SIZE, Vector2(x,100), 50, 25, [0, 0, 255])
        object_list.append(block)
        x += 60
    x2 = 38    
    for i in range(0,5):
        block = SingleHit(object_list, SCREEN_SIZE, Vector2(x2,230), 75, 25, [0, 0, 255])
        object_list.append(block)
        x2 += 85
    x3 = 25    
    for i in range(0,3):
        block = MultiHit(object_list, SCREEN_SIZE, Vector2(x3,270), 175, 25, [16, 240, 7])
        object_list.append(block)
        x3 += 185
    player_block = PlayerBlock(SCREEN_SIZE, Vector2(400,800), 150, 50, [0, 0, 0])
