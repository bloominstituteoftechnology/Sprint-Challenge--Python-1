...
class GameBall(Ball):
        ...
        if test == 1:
            ...
            for corner in corners:
                ...
                if relative_vector.length() <= self.radius:
                    ...
                    # the same speed
                    stand_in = Ball(self.bounds, corner, Vector2(
                        0, self.velocity.length()), [0, 0, 0], 0)
    ...
    def check_collision(self):
        ...
        for object in self.object_list:  # TODO: Check effeciency
            # Balls colliding with blocks
            # if (issubclass(type(object), Paddle)) and object != self:
